...
export async function POST(request: NextRequest) {
  try {
    ...
    if (redirect_uri && responseType && client_id) {
      ...
      if (!validURI) {
        return NextResponse.json(
          {
            message:
              "The provided redirect URI is Does NOT match with any of the provided URIs by the app client",
            success: false,
          },
          { status: 400 }
        );
      ...
      return NextResponse.json(
        { code: code, message: "Created succcessfully", success: true, mode: "oauth" },
        { status: 201 }
      );
    } else {
      const tokenData = {
